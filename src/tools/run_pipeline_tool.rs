...
impl Tool for RunPipelineTool {
    ...
    async fn execute(&self, _ctx: Arc<dyn ToolContext>, args: Value) -> Result<Value> {
        ...
        let (tasks_completed, message) = match &status {
            crate::agents::CompletionStatus::Complete {
                tasks_completed,
                message,
                ..
            } => (*tasks_completed, message.clone()),
            crate::agents::CompletionStatus::MaxIterationsReached {
                tasks_completed,
                tasks_remaining,
                ..
            } => (
                *tasks_completed,
                format!("{} tasks remaining", tasks_remaining),
            ),
            crate::agents::CompletionStatus::AllTasksBlocked {
                tasks_completed,
                reason,
                ..
